...
if nargin < 3
    ...
    if isscalar(varargin{2}) || ischar(varargin{2})
        ...
        values = varargin{2}; %changed it from 1 to 2
        %xOrder = 1:size(values,1); 
        xOrder=varargin{1}; %new
    else
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        if col==2
            errorbar(mean(x,1), values(xOrder,col), lowerErrors(xOrder,col), zeros(size(upperErrors(xOrder, col))), '.w')
        end
        set(hErrorbar(col), 'marker', 'none','LineWidth',1)
    end
else
    if verLessThan('matlab', '8.4')
        % Original graphics:
    ...
    set(hErrorbar, 'marker', 'none','LineWidth',1,'CapSize',12)
end
